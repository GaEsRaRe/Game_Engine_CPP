...
EnemyAlien::EnemyAlien(float agent_width, float agent_height, glm::vec2 position, std::string texture,int type,int _uid)
...
void EnemyAlien::update(float deltaTime) {
	collider.set_position(getPosition());
	_elapsed += deltaTime;
	_position.y -= _speed;

}
glm::vec2 EnemyAlien::get_collider() {
	return _collider;
}
